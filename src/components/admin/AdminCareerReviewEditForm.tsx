...
interface AdminCareerReviewEditFormProps {
  careerReview: CareerReview;
  courses: Course[];
}

interface CareerReviewUpdate {
  title: string;
  description: string;
  youtube_url: string;
  income_text: string;
...
export default function AdminCareerReviewEditForm({
  careerReview,
  courses,
}: AdminCareerReviewEditFormProps) {
  const router = useRouter();
  const supabase = createClient();

  const [title, setTitle] = useState(careerReview.title);
  const [description, setDescription] = useState(
    careerReview.description
  );
  const [youtubeUrl, setYoutubeUrl] = useState(
    careerReview.youtube_url
  );
  const [incomeText, setIncomeText] = useState(
    careerReview.income_text
  );
  const [careerType, setCareerType] =
    useState<CareerReviewType>(careerReview.career_type);
  const [courseId, setCourseId] = useState(
    careerReview.course_id
  );
  const [status, setStatus] =
    useState<CareerReviewStatus>(careerReview.status);
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      const payload: CareerReviewUpdate = {
        title: title.trim(),
